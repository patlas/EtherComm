...
void httpsrv_server_task(void *init_ptr, void *creator) 
{
    ...
    while (1) 
    {
        ...
        if (new_sock != RTCS_SOCKET_ERROR)
        {
            ...
            error = RTCS_task_create(HTTPSRV_SESSION_TASK_NAME, server->params.server_prio, HTTPSRV_SESSION_STACK_SIZE, httpsrv_session_task, &ses_param);
            if (MQX_OK != error)
            {
...
static void httpsrv_ses_init(HTTPSRV_STRUCT *server, HTTPSRV_SESSION_STRUCT *session, const int sock)
{
    if (server && session)
    {
        if (server->params.ssl_ctx != 0)
        {
            session->ssl = RTCS_ssl_socket(server->params.ssl_ctx, sock);
        }
        session->state = HTTPSRV_SES_WAIT_REQ;
        session->sock = sock;
        session->valid = TRUE;
...
static void httpsrv_plugin_run(void *server_ptr, void *session_ptr)
{
    ...
    /* Plugin handler task is not running -> start it. */
    if (server->plugin_tid == 0)
    {
        HTTPSRV_PLUGIN_TASK_PARAM params;

        params.server = server;
        if (RTCS_task_create(HTTPSRV_PLUGIN_HANDLER_NAME, server->params.server_prio, HTTPSRV_SESSION_STACK_SIZE, httpsrv_plugin_task, &params) != RTCS_OK)
        {
            goto ERROR;
        }
    }
    ...
    /* Send message to plugin handler. */
