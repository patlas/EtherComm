...
{ /* Body */
   ...
   for (;;)
   {
      ...
      RTCS_FD_ZERO(&rfds);
      RTCS_FD_SET(tftpsrv_ptr->SRV_SOCK, &rfds);
      for(i=0; i<numtrans; i++)
      {
        RTCS_FD_SET(tftpsrv_ptr->SOCKETS[i], &rfds);
      }

      err = select(numtrans+1, &rfds, NULL, timeout);
      if((0 == err) || (RTCS_ERROR == err))
      {
        continue;
      }
      if(RTCS_FD_ISSET(tftpsrv_ptr->SRV_SOCK, &rfds)) {
         /* New request, service it */
         TFTPSRV_service_request(tftpsrv_ptr);
      }
      for(i=0; i<numtrans; i++)
      {
        if(RTCS_FD_ISSET(tftpsrv_ptr->SOCKETS[i], &rfds))
        {
