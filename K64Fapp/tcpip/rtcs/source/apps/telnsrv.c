...
#else
   ioctl(fileno(sockfd), IO_IOCTL_SET_BLOCK_MODE, &param);
...
#else
   ioctl(fileno(telnetfd), IO_IOCTL_SERIAL_SET_FLAGS, &param);
#endif
...
#else
   fprintf(active_ptr->STDOUT_STREAM, "\nRTCS v%d.%02d.%02d Telnet server\n",
          RTCS_VERSION_MAJOR, RTCS_VERSION_MINOR, RTCS_VERSION_REV);
#endif
