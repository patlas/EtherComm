...
static void ws_data_frame(WS_CONTEXT_STRUCT* context);
...
static uint32_t ws_process_api_calls(WS_CONTEXT_STRUCT *context, rtcs_fd_set **unblock);
...
static uint32_t ws_process(WS_CONTEXT_STRUCT* context)
{
    ...
    if (ws_process_api_calls(context, &unblock) != WS_ERR_PASS)
    {
        retval = WS_ERR_FAIL;
        goto EXIT;
    }
    ...
    if (ws_process_frame(context) == RTCS_ERROR)
    {
        retval = WS_ERR_FAIL;
        goto EXIT;
    }
...
EXIT:
    if (unblock != 0)
    {
        RTCS_FD_SET(context->sock, unblock);
    }
    return(retval);
}
    ...
    while (MQX_OK == _lwmsgq_receive(context->msg_queue, (_mqx_max_type *) &message, LWMSGQ_TIMEOUT_FOR | LWMSGQ_RECEIVE_BLOCK_ON_EMPTY, 0, NULL))
    {
        ...
        switch (message.command)
        {
            case WS_COMMAND_SEND:
                ...
                if (ws_send_frame(context,&message.frame) == RTCS_ERROR)
                {
                    retval = WS_ERR_FAIL;
                }
            ...
            case WS_COMMAND_CLOSE:
                ...
                if (ws_send_control_frame(context, (uint8_t*) &close_reason, sizeof(close_reason), WS_OPCODE_CLOSE) == RTCS_ERROR)
                {
                    retval = WS_ERR_FAIL;
                }
            ...
            case WS_COMMAND_RECV:
                ...
                *unblock = (rtcs_fd_set *) message.data;
                break;
            default:
                break;
        }
...
static void ws_data_frame(WS_CONTEXT_STRUCT* context)
{
    ...
    {
        plugin->on_message(plugin->cookie, *user_context);
